...
public class SMSService implements MessageListener {
	...
	public static final String SMS_CITA_TEXT = "LA CITA PARA SU MASTOGRAFIA HA SIDO PROGRAMADA PARA EL ";
    	...
    	if(message instanceof ObjectMessage){
			...
			try {
				if(msg.getObject() instanceof SMSParams){
					...
					sendSMS(params);
					Cita c = params.getCita();
					if(params.isNextDayMsg())
						c.setEnviadosms(true);
					citaPersistenceEjb.save(c);
				}
			} catch (JMSException e) {
				log.error(e.getMessage());
			} catch (Exception e) {
				log.error("Failed to communicate with the Nexmo Client: " + e.getMessage());
			}
    	}
    }
    ...
    private void constructMessage(SMSParams params){
    	...
    	if(params.isNextDayMsg())
    	{
    		params.setMsg(SMS_TEXT);
    		
    	} else
    	{
    		String msg = SMS_CITA_TEXT + 
        			df.format(result);
    		params.setMsg(msg);
    	}
    ...
    private void sendSMS(SMSParams params) throws Exception{
